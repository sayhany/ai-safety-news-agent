...
from .text_utils import canonical_title, clean_html_text
...
class ArticleDeduplicator:
    ...
    def _load_caches(self) -> None:
        """Load existing caches from disk using secure JSON format."""
        try:
            if self.embedding_cache_file.exists():
                with open(self.embedding_cache_file, encoding='utf-8') as f:
                    ...
                    for key, embedding in embeddings_data.items():
                        if isinstance(embedding, list):
                            import numpy as np
                            self.embedding_cache[key] = np.array(embedding)
                        else:
    ...
    def _save_caches(self) -> None:
        ...
        try:
            ...
            cache_data['embeddings'] = serializable_cache
    ...
    async def _get_embedding(self, text: str, article_id: str) -> np.ndarray | None:
    ...
    async def deduplicate_by_embedding(self, articles: list[dict]) -> tuple[list[dict], list[DuplicateGroup]]:
        ...
        for i, (article, embedding, article_id) in enumerate(zip(articles, embeddings, article_ids, strict=False)):
...
async def deduplicate_articles(articles: list[dict], settings: Settings,
                             llm_client: LLMClient | None = None) -> tuple[list[dict], list[DuplicateGroup]]:
